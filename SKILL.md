...
> **独立仓库**：[cocodeemo/stock-roundtable](https://github.com/cocodeemo/stock-roundtable)
>
...
│   │   └── ...
│   ├── v1.9.3-audit.md   # 本 session 完整审计报告
│   └── ...
└── demos/                # HTML 报告示例（下载后浏览器打开）
```
...
命令行快速使用：
```bash
python3 demo.py "你的问题"
python3 demo.py "你的问题" --roles graham,buffett,fisher  # 指定角色
python3 demo.py "你的问题" --model deepseek-v4-pro --rounds 3  # 指定模型和轮次
```
...
| 营收 YoY > 30% 且 毛利率 > 50% | 📐 格雷厄姆 + 🐢 龟龟 | 高增长股天然不适合防御型价值/高股息策略 |
...
- **交易所识别必须用 `len(code) == 6` 而非 `len(code) == 5`**：`fetch_stock_quote()` 的交易所识别逻辑中，深市代码（000762、002738、300308）都是 6 位，不满足 `len(code) == 5` 会落入 else 被当成港股。正确逻辑：`len(code) == 6 and starts with 6/9 → sh`；`len(code) == 6 and starts with 0/3 → sz`；其余 → hk。本 session 代码审查发现此 bug——西藏矿业(000762)会被当成 hk000762 分析，腾讯(00700)反而被当成 sz00700。
  ```
...
- **stock_debate.py 运行期间终端零输出（5-10 分钟静默）**：`stock_debate.py` 通过 `subprocess.run(capture_output=True)` 调用 `demo.py`，辩论阶段（6 角色 × 2 轮 LLM 调用）所有 stdout 被静默捕获，直到辩论结束才一次性刷出。不要在 30-60 秒后以为脚本挂了而 kill 重试——数据抓取阶段有输出（📡获取行情/📊财务数据等），之后静默是正常的。判断是否仍在运行：`ls -t ~/Desktop/report_*.html` 看报告文件是否生成，或用 `ps aux | grep stock_debate` 查进程。

- **akshare 是可选依赖**：`fetch_financial_data()` 在 akshare 未安装时返回 `{}`（空 dict），脚本自动跳过财务数据注入，仅用腾讯行情数据跑辩论。`cross_validate()` 和 `filter_roles()` 均兼容空 dict 输入。不安装 akshare 不会阻塞辩论，只是角色缺少最新财报数字（营收/净利/利润率/ROE），辩论质量下降。如需深度财务分析，先 `pip3 install akshare`。
...
- **辩论问题必须通用化，不能是单一二元判断**：旧版问题「是否值得持有？」太窄——所有角色从零开始分析买不买，但实际投资者已有持仓时视角完全不同。v1.7.1 改为三种情境：「尚未持有：是否买入？/ 已持有：持有、加仓还是减仓？/ 无论哪种：风险收益比如何？」。这样莫大(重仓华特气体)会自然以持有者视角给结论，而不是上来就 37 分说「离开这个票方得心安」。本 session 用户指出笨韭和莫大实际重仓华特气体后才意识到问题。\n\n- **Hermes config.yaml 的 custom_providers 是 list 不是 dict**：v1.7 的 `yaml.safe_load()` 解析器第一版只遍历了 `cfg['providers']` (dict)，但实际 config 中 API key 在 `cfg['custom_providers']` (list) 里——`[{name: 'taotoken.net', api_key: 'sk-...'}]`。直接按 dict 遍历 → 找不到 key → 报「❌ 未找到 API key」。修复后兼容 5 种 config 结构：顶层 direct key / providers dict / custom_providers list / delegation.api_key / api_server.key。

- **cfg 变量作用域陷阱**：`main()` 中 api_key 读取的 `except` 分支不会定义 `cfg`，后续 api_base 读取 `isinstance(cfg, dict)` 会触发 NameError。修复：在 try 前初始化 `cfg = None`。本 session 代码审查发现此 bug。

- **_parse() NaN 传播**：pandas DataFrame 的 NaN 是 `float` 类型，`isinstance(NaN, float)` 返回 True → `_parse()` 直接返回 NaN → f-string 输出 "nan"。修复：`if isinstance(v, float) and math.isnan(v): return 0`。本 session 代码审查发现此 bug。

- **custom_providers 中 name 字段可能不带 `.net` 后缀**：部分用户 config 里 custom_providers 的 name 是 `'taotoken'` 而非 `'taotoken.net'`。API key 提取已改为不按 name 匹配——遍历所有 provider 只要有 `api_key` 字段就用。API base URL 从 config.yaml → env var `TAOTOKEN_API_BASE` → fallback `https://taotoken.net/api/v1`，与 name 字段无关。

- **✅ 重复 import（v1.9.7 已修复）**：`stock_debate.py` 的 `fetch_eastmoney_quote()` 和 `fetch_industry_context()` 内部各自有 `import json, urllib.request`，模块顶部 L13 已导入。v1.9.7 已删除函数内重复 import。

- **✅ `type()` hack 假 Match 对象（v1.9.7 已修复）**：`html_report.py` `_build_comparison_table()` 在正则提取分数失败时，曾用 `type('m', (), {'group': ...})()` 创建假 `re.Match` 对象兜底。v1.9.7 改为 `_table_score = v` + `score = None` → `elif _table_score is not None:` 分支，不再依赖 `re.Match` 内部结构。

- **✅ `_visualize_scores()` 闭包内函数定义（v1.9.7 已修复）**：`add_bar_to_row` 从 `convert_score_table` 内部提升到 `_visualize_scores` 同层（改名为 `_add_bar_to_row`），避免每次匹配重新创建函数对象。

- **🟡 `html` 变量名遮盖 import（v1.9.7 已修复）**：`render_html()` 末尾用 `html = f"""...` 组装最终 HTML，导致 line 771 的 `html.escape()` 报 `UnboundLocalError`——Python 把 `html` 视为局部变量。修复：将局部变量改名为 `full_html`，保留 `import html` 模块可用。这个 bug 预存已久但 `__main__` 测试块的 `render_html(result, ...)` 调用从未进入（被 Linux 路径硬编码挡住）所以未被发现。

v1.9.8 (2026-06-30) — 运行期运维经验沉淀：
- 新增 pitfall：stock_debate.py 运行期间终端零输出（subprocess capture_output=True 导致 5-10 分钟静默），不要误判为挂死
- 新增 pitfall：akshare 是可选依赖，未安装时脚本优雅降级（仅用行情数据辩论）
- 实战验证：紫金矿业(601899) 全流程跑通，6 角色 × 2 轮，报告输出 ~/Desktop/report_紫金矿业_601899_20260629.html
v1.9.7 (2026-06-30) — v1.9.6 审查发现的 3 项修复 + 预存 `html` shadowing bug 修复：
- ✅ 删除 stock_debate.py 两处函数内重复 `import json, urllib.request`
- ✅ 重构 html_report.py `_build_comparison_table()` — 移除 `type('m',(),{...})()` 假 Match 对象
- ✅ 提升 html_report.py `_visualize_scores()` 闭包函数 `_add_bar_to_row` 到平级
- ✅ 修复 html_report.py `render_html()` 中 `html` 变量遮盖 import（预存 UnboundLocalError）
v1.9.5 (2026-06-30) — 代码审查修复（5项）：
- L1 py_compile：5文件全通过 | L2 AST跨文件契约：to_dict()↔render_html键匹配、filter_roles()元组解包正确
- L3 逻辑边界：交易所识别(8用例)、_parse() NaN(9用例)、is_growth非死代码、cfg作用域 — 全部通过
- L4 安全：无硬编码key、无shell=True、html.escape覆盖display_title+recommendation、CSS无尾部逗号
- 发现3项非关键问题（不阻塞，记录在此供后续优化）：
    🟡① stock_debate.py L152+L257 重复 import json, urllib.request（模块级已导入，函数内重复）
    🟡② html_report.py _build_comparison_table() 用 type('m',(),{...})() 假Match对象兜底（脆弱，Python升级可能破坏）
    🟡③ html_report.py _visualize_scores() 闭包内定义 convert_score_table/add_bar_to_row（每次匹配重新创建函数对象）
